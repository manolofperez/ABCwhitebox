...
calculate_sust <- function(loc){
  if (loc == nloc){
    ct_ms<-F_ST.stats(ct_ms)
    ct_ms<-diversity.stats(ct_ms)
    ct_ms<-neutrality.stats(ct_ms)
    ct_ms@region.stats@nucleotide.diversity
    get.diversity(ct_ms)[,1]
    get.F_ST(ct_ms)
    get.neutrality(ct_ms)[,1]
  }
  else
    if (loc == ctloc){
      ct_ms<-F_ST.stats(ct_ms)
...
calculate_sust <- function(seq_genome){
  if (seq_genome == nDNA){
    ct_ms<-F_ST.stats(ct_ms)
    ct_ms<-diversity.stats(ct_ms)
    ct_ms<-neutrality.stats(ct_ms)
